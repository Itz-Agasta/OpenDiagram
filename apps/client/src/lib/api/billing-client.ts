...
const BASE = `${BASE_URL.replace(/\/$/, "")}/api/billing`;
...
export async function getBillingState(): Promise<BillingState> {
  const response = await devFetch(BASE, { credentials: "include" });
...
export async function startCheckout(discountCode?: string): Promise<string> {
  const response = await devFetch(`${BASE}/checkout`, {
    method: "POST",
    credentials: "include",
...
export async function reconcileCheckout(subscriptionId: string): Promise<{
  ...
  const response = await devFetch(`${BASE}/reconcile`, {
    method: "POST",
    credentials: "include",
    headers: { "Content-Type": "application/json" },
...
export async function openBillingPortal(): Promise<string> {
  const response = await devFetch(`${BASE}/portal`, {
    method: "POST",
    credentials: "include",
  });
