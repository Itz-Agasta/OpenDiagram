...
import { isAskUserPart, type ChatToolPart } from "./diagram-chat";
...
function toolPartToStoredAskUser(part: ChatToolPart): StoredChatPart | null {
  ...
  if (part.state === "output-error") {
    return {
      ...
      state: "output-error",
      ...(isAskUserInput(part.input) ? { input: part.input } : {}),
      errorText: typeof part.errorText === "string" ? part.errorText : "Question failed.",
    };
  }
...
function uiPartToStoredPart(part: UIMessage["parts"][number]): StoredChatPart | null {
  ...
  if (!isAskUserPart(part)) return null;
  return toolPartToStoredAskUser(part);
}
