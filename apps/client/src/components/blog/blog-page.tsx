...
import type { BlogPostSummary, BlogTag } from "#/lib/types/blog";
import { GITHUB_URL } from "#/lib/utils/site";
import { BlogAuthor } from "#/components/blog/blog-author";
import { MarketingButton } from "#/components/ui/button";

const BLOG_LIST_DATE_FORMATTER = new Intl.DateTimeFormat("en-US", {
  dateStyle: "medium",
  timeZone: "UTC",
});
        ...
        <section className="px-6 py-12 md:px-12 lg:px-[120px]">
          <div className="mx-auto grid w-full max-w-[1200px] gap-7 md:grid-cols-2">
            {filteredPosts.map((post) => (
              <article
                key={post.href}
                className="group overflow-hidden rounded-[24px] border border-black/[0.08] bg-white shadow-[0_12px_30px_rgba(0,0,0,0.025)] transition-[border-color,box-shadow] duration-300 hover:border-black/15 hover:shadow-[0_20px_40px_rgba(0,0,0,0.05)]"
              >
                  ...
                  <div className="p-7 md:p-8">
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1 font-mono text-[9px] uppercase tracking-[0.16em] text-black/42">
                      <time dateTime={post.date}>
                        {BLOG_LIST_DATE_FORMATTER.format(new Date(`${post.date}T00:00:00Z`))}
                      </time>
...
function BlogEmptyState({
  ...
  return (
    <section className="px-6 py-12 md:px-12 lg:px-[120px]">
      <div className="mx-auto w-full max-w-[1200px]">
        <div className="relative overflow-hidden rounded-[24px] border border-dashed border-black/15 bg-white p-8 md:p-16 text-center shadow-[0_8px_30px_rgb(0,0,0,0.02)] transition-[border-color] duration-300 hover:border-black/30">
          ...
          <div className="relative z-10 flex flex-col items-center max-w-[620px] mx-auto">
            ...
            {/* Title */}
            <h2 className="text-[28px] font-medium leading-tight tracking-[-0.03em] text-[#1a1a1a] md:text-[36px]">
              {title}
            </h2>

            {/* Description */}
            <p className="mt-4 text-base leading-[1.65] text-black/60 md:text-lg">{description}</p>

            {/* Inline call to action */}
            <div className="mt-10 w-full max-w-[460px]">
              <Link to="/login">
                <MarketingButton text="Get started free" color="black" />
              </Link>
            </div>
          </div>
        </div>
